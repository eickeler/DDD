...
#define SIGNAL_ARROW "signal_arrow"
#define DRAG_ARROW "drag_arrow"
#define PLAIN_STOP "plain_stop"
#define PLAIN_COND "plain_cond"
#define PLAIN_TEMP "plain_temp"
#define GREY_STOP "grey_stop"
#define GREY_COND "grey_cond"
#define GREY_TEMP "grey_temp"
...
// Models for the ListStore for Breakpoints
class BreakpointListColumns: public Gtk::TreeModel::ColumnRecord
{
public:
    Gtk::TreeModelColumn<int> num;
    Gtk::TreeModelColumn<const char *> type;
    Gtk::TreeModelColumn<bool> temporary;
    Gtk::TreeModelColumn<bool> enabled;
    Gtk::TreeModelColumn<void *> address;
    Gtk::TreeModelColumn<Glib::ustring> what;
...
static BreakpointListColumns *
breakpoint_list_columns_p(void)
{
    static BreakpointListColumns *blc = NULL;
    if (!blc)
	blc = new BreakpointListColumns;
    return blc;
}

#define breakpoint_list_columns (*breakpoint_list_columns_p())

// Models for the ListStore for Registers
class RegisterListColumns: public Gtk::TreeModel::ColumnRecord
{
public:
    Gtk::TreeModelColumn<const char *> name;
    Gtk::TreeModelColumn<void *> hex;
    Gtk::TreeModelColumn<long> dec;
    RegisterListColumns()
	{
	    add(name);
	    add(hex);
...
static RegisterListColumns *
register_list_columns_p(void)
{
    static RegisterListColumns *rlc = NULL;
    if (!rlc)
	rlc = new RegisterListColumns;
    return rlc;
}

#define register_list_columns (*register_list_columns_p())
...
#else
GUI::Container *SourceView::toplevel_w       = 0;
GUI::Container *SourceView::source_form_w    = 0;
GUI::ScrolledText *SourceView::source_text_w = 0;
#endif
#if defined(IF_XM)
...
// Return true if W is a descendant of source_form_w
bool SourceView::is_source_widget(GUI::Widget *w)
{
    while (w != 0)
    {
	if (w == source_form_w)
...
string& SourceView::current_text(GUI::Widget *w)
{
    assert(is_source_widget(w) || is_code_widget(w));
...
void SourceView::line_popup_set_tempCB (GUI::Widget *w, const string *address)
{
    create_temp_bp(*address, w);
}
...
void SourceView::temp_n_cont(const string& a, GUI::Widget *w)
{
    CommandGroup cg;
...
bool SourceView::move_bp(int bp_nr, const string& a, GUI::Widget *w, bool copy)
{
    CommandGroup cg;
...
void SourceView::disable_bps(const IntArray& nrs, GUI::Widget *w)
{
    CommandGroup cg;
...
void SourceView::delete_bps(const IntArray& nrs, GUI::Widget *w)
{
    CommandGroup cg;
...
void SourceView::text_popup_clearCB (GUI::Widget *w, const string *word_ptr)
{
    clear_bp(fortranize(*word_ptr, true), w);
}
#endif
			    ...
			    bool silent)
{
    ...
    */
	...
	if (!popped_up)
	{
		...
		gdbOpenSourceWindowCB(Widget(0), XtPointer(0), XtPointer(0));
#else
	    GUI::Shell *shell = (source_view_shell != 0) ? 
		source_view_shell : command_shell;
...
void SourceView::srcpopupAct (GUI::Widget *w, GUI::Event* e, GUI::String *, unsigned int *)
{
	...
	static GUI::Widget *bp_popup_parent = 0;
...
#warning MMaddCallbacks cannot override existing closure callback_word.
	...
	if (current_arg != current_ref_arg)
	{
	    text_popup[TextItms::Sep1].widget->show();
	    text_popup[TextItms::PrintRef].widget->show();
	    text_popup[TextItms::DispRef].widget->show();
	    // XtManageChild(text_popup[TextItms::WatchRef].widget);
	}
	else
	{
	    text_popup[TextItms::Sep1].widget->hide();
	    text_popup[TextItms::PrintRef].widget->hide();
	    text_popup[TextItms::DispRef].widget->hide();
	    text_popup[TextItms::WatchRef].widget->hide();
	}
...
void SourceView::update_properties_panel(BreakpointPropertiesInfo *info)
{
	...
	bool lock = info->spin_locked;
	info->spin_locked = true;
	GUI::SpinButton *sb = dynamic_cast<GUI::SpinButton *>(info->ignore);
	assert(sb);
...
void SourceView::edit_bps(IntArray& breakpoint_nrs, Widget /* origin */)
{
	...
	GENTRYLI("lookup", N_("Lookup"), LOOKUP_ICON, MMPush,
		...
		0, &info->lookup),
	GENTRYLI("print", N_("Print XXX"), PRINT_ICON, MMPush,
	...
	GENTRYLI("disable", N_("Disable"), DISABLE_ICON, MMPush,
		...
		0, &info->disable),
	GENTRYLI("temporary", N_("Temporary"), MAKETEMP_ICON, MMPush,
	...
	GENTRYL("condition", N_("Condition"), MMComboBox,
		...
		MDUMMY, /* sigc::bind(sigc::ptr_fun(SetBreakpointConditionCB), info), */
		0, &info->condition),
...
void SourceView::edit_bps(IntArray& breakpoint_nrs, GUI::Widget * /* origin */)
{
    ...
    // FIXME: We should not call SetBreakpointConditionCB on every
    // change to the ComboBox.  Only pressing "Enter" or selecting
    // from the drop-down list should call this (otherwise we end up
    // trying to send a partial expression to GDB).
    MMDesc panel_menu[] = 
    {
	GENTRYL("title", N_("Title"), MMButtonPanel,
		MMNoCB, MDUMMY, enabled_menu, 0),
    ...
    info->editor = new GUI::ScrolledText(*form);
    info->editor->show();
...
#endif
    info->dialog->signal_unmap().connect(sigc::bind(sigc::ptr_fun(DestroyThisCB), info->dialog));
    info->dialog->add_destroy_notify_callback(info, (void *(*)(void *))DeleteInfoCB);
...
// Enable Breakpoints
void SourceView::EnableBreakpointsCB(GUI::Widget *w, BreakpointPropertiesInfo *info)
{
    enable_bps(info->nrs, w);
}
...
#endif
{
	...
	gdb_command(gdb->frame_command(item_position - 1));
	break;
...
{
    static GUI::connection check_scroll_id;
    ...
    check_scroll_id = GUI::signal_timeout().connect(sigc::bind(sigc::ptr_fun(CheckScrollWorkProc),
							       &check_scroll_id),
						    app_data.glyph_update_delay);
...
#ifdef NAG_ME
#warning HEIGHT OF TEXTVIEW?
#endif
    std::cerr << "FIXME: Height of textview\n";
    int end_x, end_y;
    text_w->view().window_to_buffer_coords(Gtk::TEXT_WINDOW_TEXT,
					   0, text_w->view().get_height(), end_x, end_y);
    Gtk::TextIter iter;
    text_w->view().get_iter_at_location(iter, 0, 0);
    current_top = iter.get_offset();

    while (iter != text_w->buffer()->end()) {
      int y, height;
      text_w->view().get_line_yrange(iter, y, height);
      if (y < end_y) break;
    }
	...
	pos = min(current_bottom + 1, textlen);
